...
def get_schedule(day_filter=None, subject_filter=None, section_filter=None, room_filter=None):
    ...
    with sqlite3.connect(DB_NAME) as conn:
        ...
        # Step 1: Find all rooms that are occupied at the current time
        for row in rows:
            id = row[0]
            schedule_day = row[1]
            start_time = row[2]
        ...
        # Step 2: Loop through the schedule and apply status logic
        for row in rows:
            id = row[0]
            schedule_day = row[1]
            start_time = row[2]
        ...
        # Sort by availability, with "Available" rooms first
